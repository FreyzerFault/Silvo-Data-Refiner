...
from utils.file_manager import get_files_by_extension, print_files, read_csv, write_csv
from utils.utils import colorize
from utils.config import Config

from data_operations.refactor import refactor_data_file, refactor, add_end_date
...
sort_by_columns = pipeline['sort_by']
...
def main():
  ...
  in_file_paths = get_files_by_extension(in_data_root)
  for in_file_path in in_file_paths:
    df = read_csv(in_file_path)
    df = refactor(df)
    df = sort_by(df, sort_by_columns)
    df = merge(df)
    df = add_end_date(df)
    df = group_by(df)
